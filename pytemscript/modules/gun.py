...
class Gun:
    """ Gun functions. """
    __slots__ = ("__client", "__has_gun1", "__has_source", "__err_msg_gun1", "__err_msg_cfeg")
    ...
    def __gun1_available(self) -> bool:
        if self.__has_gun1 is None:
            self.__has_gun1 = self.__client.has("tem.Gun1")
        return self.__has_gun1

    @property
    def __adv_available(self) -> bool:
        if self.__has_source is None:
            self.__has_source = self.__client.has("tem_adv.Source.State")
        return self.__has_source

    @property
    def shift(self) -> tuple:
        """ Gun shift. (read/write)"""
        return (self.__client.get("tem.Gun.Shift.X"),
                self.__client.get("tem.Gun.Shift.Y"))

    @shift.setter
    def shift(self, values: tuple) -> None:
        new_value = Vector(*values)
        new_value.set_limits(-1.0, 1.0)
        self.__client.set("tem.Gun.Shift", new_value)

    @property
    def tilt(self) -> tuple:
        """ Gun tilt. (read/write)"""
        return (self.__client.get("tem.Gun.Tilt.X"),
                self.__client.get("tem.Gun.Tilt.Y"))

    @tilt.setter
    def tilt(self, values: tuple) -> None:
        new_value = Vector(*values)
        new_value.set_limits(-1.0, 1.0)
        self.__client.set("tem.Gun.Tilt", new_value)

    @property
    def voltage_offset(self) -> float:
        """ High voltage offset. (read/write)"""
        if self.__gun1_available:
            return self.__client.get("tem.Gun1.HighVoltageOffset")
        else:
    ...
    def voltage_offset(self, offset: float) -> None:
        if self.__gun1_available:
            self.__client.set("tem.Gun1.HighVoltageOffset", float(offset))
        else:
            raise NotImplementedError(self.__err_msg_gun1)
    ...
    def feg_state(self) -> str:
        ...
        if self.__adv_available:
            return FegState(self.__client.get("tem_adv.Source.State")).name
        else:
    ...
    def ht_state(self) -> str:
        ...
        """
        return HighTensionState(self.__client.get("tem.Gun.HTState")).name

    @ht_state.setter
    def ht_state(self, value: HighTensionState) -> None:
        self.__client.set("tem.Gun.HTState", value)
    ...
    def voltage(self) -> float:
        ...
        """
        state = self.__client.get("tem.Gun.HTState")
        if state == HighTensionState.ON:
            return self.__client.get("tem.Gun.HTValue") * 1e-3
        else:
            return 0.0
    ...
    def voltage(self, value: float) -> None:
        ...
        self.__client.set("tem.Gun.HTValue", float(value) * 1000)
        while True:
            if self.__client.get("tem.Gun.HTValue") == float(value) * 1000:
    ...
    def voltage_max(self) -> float:
        ...
        return self.__client.get("tem.Gun.HTMaxValue") * 1e-3
    ...
    def voltage_offset_range(self):
        ...
        if self.__gun1_available:
            ...
            return self.__client.call("tem.Gun1.GetHighVoltageOffsetRange()")
        else:
            raise NotImplementedError(self.__err_msg_gun1)
    ...
    def beam_current(self) -> float:
        ...
        if self.__adv_available:
            return self.__client.get("tem_adv.Source.BeamCurrent")
        else:
            raise NotImplementedError(self.__err_msg_cfeg)
    ...
    def extractor_voltage(self) -> float:
        ...
        if self.__adv_available:
            return self.__client.get("tem_adv.Source.ExtractorVoltage")
        else:
            raise NotImplementedError(self.__err_msg_cfeg)
    ...
    def focus_index(self) -> tuple:
        ...
        if self.__adv_available:
            return (self.__client.get("tem_adv.Source.FocusIndex.Coarse"),
                    self.__client.get("tem_adv.Source.FocusIndex.Fine"))
        else:
            raise NotImplementedError(self.__err_msg_cfeg)
    ...
    def do_flashing(self, flash_type: FegFlashingType) -> None:
        ...
        if not self.__adv_available:
            raise NotImplementedError(self.__err_msg_cfeg)
        if self.__client.call("tem_adv.Source.Flashing.IsFlashingAdvised()", flash_type):
            # FIXME: lowT flashing can be done even if not advised
            self.__client.call("tem_adv.Source.Flashing.PerformFlashing()", flash_type)
        else:
