...
# ---------------- TFS Gatan Remoting enums -----------------------------------
class AcquisitionMode(IntEnum):
    """ Gatan camera acquisition mode. """
    SINGLE = 1
    CONTINUOUS = 2


class AcquisitionProcessingOption(IntEnum):
    """ Gatan camera image processing. """
    UNPROCESSED = 1
    DARKSUBTRACTED = 2
    GAINNORMALIZED = 3
...
class AcquisitionReadMode(IntEnum):
    """ Gatan camera operation mode. """
    LINEAR = 0
    COUNTING = 1
    COUNTING_AND_SUPERRESOLUTION = 2


class FractionsFileFormat(IntEnum):
    """ Gatan movies file format. """
    MRC_STACK = 0
    TIFF = 1
    TIFF_LZW = 2
...
class TransposeSetting(IntEnum):
    ...
    ROTATE270_FLIPX = 259
    ROTATE270_FLIPY = 256
    ROTATE270_FLIPXY = 258


class ShutterControl(IntEnum):
    """ Gatan camera shutter position. """
    UNKNOWN = 0
    PRESPECIMEN = 1
    POSTSPECIMEN = 2
    GIF = 3
...
class QualityLevel(IntEnum):
    """ Gatan image quality level. """
    FAST = 0
    NORMAL = 1


class SharedMemoryBuffer(IntEnum):
    """ Gatan image shared memory buffer. """
    BUFFER1 = 1
    BUFFER2 = 2


class AcquisitionEventId(IntEnum):
    """ Gatan image acquisition event id. """
    EXPOSURE_FINISHED = 1
    STORAGE_FINISHED = 2
    ACQUISITION_FINISHED = 3
    IMAGE_AVAILABLE = 4
